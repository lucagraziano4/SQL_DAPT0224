...
il nome del reseller e l’area geografica. (seguendo i collegamenti da dimgeography a dimproductcategory)*/
...
/* iniziando da FactResellerSales passando prima da dimreseller e dimgeography.*/
SELECT A.SalesOrderNumber, A.SalesOrderLineNumber, A.OrderDate, 
A.UnitPrice, A.OrderQuantity, A.TotalProductCost,
D.EnglishProductName, F.EnglishProductCategoryName, 
B.ResellerName, C.GeographyKey, 
C.City, C.EnglishCountryRegionName
FROM FactResellerSales A
JOIN DimReseller B ON A.ResellerKey = B.ResellerKey
JOIN dimgeography C ON B.GeographyKey = C.GeographyKey
JOIN DimProduct D ON A.ProductKey = D.ProductKey
JOIN DimProductSubcategory E ON D.ProductSubcategoryKey = E.ProductSubcategoryKey
JOIN DimProductCategory F ON E.ProductcategoryKey = F.ProductcategoryKey;

/*iniziando da factresellersales passando prima da dimproduct,etc*/
SELECT A.SalesOrderNumber, A.SalesOrderLineNumber, A.OrderDate, 
A.UnitPrice, A.OrderQuantity, A.TotalProductCost,
B.EnglishProductName, D.EnglishProductCategoryName, 
E.ResellerName, F.GeographyKey, 
F.City, F.EnglishCountryRegionName
FROM FactResellerSales A
JOIN DimProduct B ON A.ProductKey = B.ProductKey
JOIN DimProductSubcategory C ON B.ProductSubcategoryKey = C.ProductSubcategoryKey
JOIN DimProductCategory D ON C.ProductcategoryKey = D.ProductcategoryKey 
JOIN DimReseller E ON A.ResellerKey = E.ResellerKey
JOIN dimgeography F ON E.GeographyKey = F.GeographyKey;